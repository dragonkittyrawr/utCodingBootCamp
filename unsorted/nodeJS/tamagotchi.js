...
cat.destroyFurniture = function() {
    if (this.houseCondition > 0) {
        ...
        console.log("Your furniture has been destroyed!")
    }
};

cat.buyNewFurniture = function() {
    this.houseCondition += 50;
    console.log("Are you sure about that? It's just going to get destroyed...");
};
...
inquirer.prompt([{
    type: "list",
    message: "What kind of DigitalPal do you want?",
    name: "digiPal",
    choices: ["dog", "cat"]
}]).then(function(question1) {
    if (question1.digiPal === "dog") {
        dog.bark();
    } else {
        cat.meow();
    }
    ...
    inquirer.prompt([{
        type: "input",
        message: "Name your DigitalPal:",
        name: "palName"
    }]).then(function(question2) {
        console.log(question2.palName);

        inquirer.prompt([{
            type: "confirm",
            message: "Play with " + question2.palName,
            name: "playConfirm"
        }]).then(function(user) {
            if (user.playConfirm === true && question1.digiPal === "dog") {
                inquirer.prompt([{
                    type: "list",
                    message: "Pick a command:",
                    name: "command",
                    choices: ["feed", "sleep", "play", "go outside"]
                }]).then(function(user) {
                    playTime();
                })
            } else if (user.playConfirm === true && question1.digiPal === "cat") {
                inquirer.prompt([{
                    type: "list",
                    message: "Pick a command:",
                    name: "command",
                    choices: ["feed", "sleep", "play", "scratch"]
                }]).then(function(user) {
                    playTime();
                })
            }
